        ...
        if let Some(displayed_popup) = self.displayed_popup {
            match displayed_popup {
                ...
                InGameViewPopup::RestartGameConfirmation => {
                    RestartGameConfirmationPopupUi::default().render(body, buf)
                }
            }
        }
...
#[derive(Default)]
pub struct RestartGameConfirmationPopupUi {}
impl Widget for RestartGameConfirmationPopupUi {
    fn render(self, area: Rect, buf: &mut Buffer)
    where
        Self: Sized,
    {
        let popup_block = Block::default()
            .title(" Restart Game Confirmation ")
            .borders(Borders::ALL)
            .style(Style::default().bg(Color::Black));

        let text = vec![
            Line::from(Span::styled(
                "⚠ You are about to restart the game",
                Style::default()
                    .fg(Color::Yellow)
